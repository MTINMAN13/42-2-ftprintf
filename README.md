...
issue was fixed for Pointer Printout calculation

solution was introduction of robust pointer handler which immediately counts the written chars

before i used separate functions for calc and for printing, leading to unexpected behavior in extreme ccases

furthermore, fixed a segmentation fault which would occur when passing (char *)NULL to string handler

> #### issue of Dereferencing a Null Pointer
> Dereferencing a null pointer:
>
> Attempting to access or modify data through a pointer that hasn't been initialized or has been set to NULL.
